...
@section('content')
    <section class="relative flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-100 to-gray-300 dark:from-gray-800 dark:to-gray-900">
        ...
        <div class="text-center bg-white dark:bg-gray-800 shadow-lg rounded-lg p-10 max-w-lg">
            <h1 class="text-8xl font-extrabold text-primary-600 dark:text-primary-500">403</h1>
            <p class="text-4xl font-bold text-gray-900 dark:text-white mt-4">Access Denied</p>
            <p class="text-lg text-gray-600 dark:text-gray-300 mt-2">Oops! You don’t have permission to access this page.</p>
            
            @if($exception->getMessage())
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">{{ $exception->getMessage() }}</p>
            @endif
